...
user_of_every_modifications = lines.map(lambda line: extract_colors(line))
result = user_of_every_modifications.reduceByKey(lambda a, b: a + b)
sorted_result = result.sortBy(lambda x: x[1], ascending=False)
formated_result = sorted_result.map(lambda x: x[0] + "," + str(x[1]))
top_10 = sc.parallelize(formated_result.take(10))
top_10.coalesce(1).saveAsTextFile(output_file)